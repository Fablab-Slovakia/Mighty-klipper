# Support for SPI based ADS1118 ADC temperature sensors
#
...
#from sigit import TemperatureSingleton


ADS1118_REPORT_TIME = .5
...
class ADS1118:
    def __init__(self, config):
        #self.temps = TemperatureSingleton.instance()
        self.printer = config.get_printer()
        self.name = config.get_name().split()[-1]
        self.reactor = self.printer.get_reactor()
        self.spi = bus.MCU_SPI_from_config(
            config, 0, pin_option="sensor_pin", default_speed=4000000)
        self.mcu = self.spi.get_mcu()
        self.temp = self.min_temp = self.max_temp = 0.0
        self.channel = config.getint('ads1118_channel')
        self.sample_timer = self.reactor.register_timer(self._sample_ads1118)
        self.printer.add_object("ads1118 " + self.name, self)
    ...
    def get_report_time_delta(self):
        return ADS1118_REPORT_TIME

    def _init_ads1118(self):
        ...
        try:
            ...
            logging.info("ads1118: initialized")
        except:
            pass

    def _sample_ads1118(self, eventtime):
        try:
            self.temp  = self.read_temp(self.channel)
            #logging.info("temp: %f", self.temp)
        except Exception:
            logging.exception("ads1118: Error reading data")
            self.temp = 0.0
            return self.reactor.NEVER

        if self.temp < self.min_temp or self.temp > self.max_temp:
            self.printer.invoke_shutdown(
                "ADS1118 temperature %0.1f outside range of %0.1f:%.01f"
                % (self.temp, self.min_temp, self.max_temp))

        measured_time = self.reactor.monotonic()
        self._callback(self.mcu.estimated_print_time(measured_time), self.temp)
        return measured_time + ADS1118_REPORT_TIME
    ...
    def read_temp(self, channel):
        ...
        raw_tc_res = self.spi.spi_transfer([(CHANNEL_2_CFG | START_SINGLE_SHOT) >> 8, (CHANNEL_2_CFG | START_SINGLE_SHOT) & 0xff])
        self.reactor.pause(self.reactor.monotonic() + .06)
        ...
        raw_tc2_res = self.spi.spi_transfer([(INTERNAL_T | START_SINGLE_SHOT) >> 8, (INTERNAL_T | START_SINGLE_SHOT) & 0xff])
        self.reactor.pause(self.reactor.monotonic() + .06)
        ...
        raw_tc2 = bytearray(raw_tc2_res['response'])
        raw_tc2 = (raw_tc2[0] << 8) | raw_tc2[1]
        tc2_uv = self._raw_to_uv(raw_tc2)


        #self.temps.set(1, (self._tc_cj_comp( tc_uv, it_c ) / 1000.0))
        #self.temps.set(2, (self._tc_cj_comp( tc2_uv, it_c ) / 1000.0))
        ads1118helper.temp1 = (self._tc_cj_comp( tc_uv, it_c ) / 1000.0)
        ads1118helper.temp2 = (self._tc_cj_comp( tc2_uv, it_c ) / 1000.0)


        if channel == 1:
            return ads1118helper.temp1
        if channel == 2:
            return ads1118helper.temp2

        else:
            self.printer.invoke_shutdown("ADS1118 invalid channel settings %s" % (channel))
        return 0
...
def load_config(config):
    ...
    pheaters.add_sensor_factory("ADS1118", ADS1118)
